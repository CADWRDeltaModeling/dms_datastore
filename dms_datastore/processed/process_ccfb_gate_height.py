...
def read_wonderware(infile):
    ...
    ts.index = ts.index.tz_localize(
        pdt, nonexistent="shift_backward", ambiguous="infer"
    ).tz_convert(pst)
...
def ccf_trim(df, outfile):
    ...
    df2dup = (df2 == 0.0).all(axis=1) & (df2 == df2.shift(1)).all(axis=1)
    df2dup |= (df2sum != 0.0) & ((df2 - df2.shift(1)).abs() <= 0.01).all(axis=1)
    df2 = df2.loc[~df2dup, :]
...
def ccfb_th_line(tm, nduplicate, height):
    ...
    finalline = startline.format(
        datestr, install, int(nduplicate), op_down, op_up, elev, width, abs(height)
    )
    return finalline
...
def invert_flow(flow, x1, x2):
    ...
    """
    safe_flow = np.minimum(np.maximum(0.0, flow.data), (x1 * x1) / (-4.0 * x2))
    m = np.sqrt(x1 * x1 + 4.0 * x2 * safe_flow)
    hgate = 0.0 * flow
    hgate_minus = (-x1 - m) / (2.0 * x2)
    hgate_plus = (-x1 + m) / (2.0 * x2)
...
def read_gate_height(datafile, thresh_open=0.03):
    ...
    df["height"] = height_sum.divide(df.ndup, axis="index", fill_value=0.0) * units.FT2M
...
def process_ccfb_gate(
    ...
    if basefile is not None:
        ...
        existing = pd.read_csv(
            basefile, sep="\s+", index_col=0, comment="#", parse_dates=[0], header=0
        )[["ndup", "height"]]
        ...
        height_ts = pd.concat(
            (
                existing.loc[slice(None, transition), ["ndup", "height"]],
                height_ts.loc[slice(transition, None), ["ndup", "height"]],
            ),
            axis=0,
        )
    ...
    if appendfile is not None:
        ...
        pred = pd.read_csv(
            appendfile, sep="\s+", index_col=0, comment="#", parse_dates=[0], header=0
        )[["ndup", "height"]]
        ...
        height_ts = pd.concat(
            (
                ...
                pred.loc[slice(append_transition, None), ["ndup", "height"]],
            ),
            axis=0,
        )
        duplicatetime = height_ts.index.duplicated(keep="first")
...
@click.option(
    "--appendfile", type=str, default=None, help="name of prediction file to append"
)
@click.option(
    "--append-transition",
    "append_transition",
...
@click.option(
    ...
    "prepro_out",
    type=str,
    default="ccf_prepro.csv",
    help="name of intermediate output file for preprocessor",
)
def process_ccfb_gate_cli(
    infile,
    outfile,
    preprocess,
    prepro_out,
