...
def process_bbid(bbid_csv, bbid_bak, interval, stime, etime):
    ...
    bbid_fine = rhistinterp(bbid + 5.0, interval, lowbound=0.0, p=20.0) - 5.0
...
def write_csv(fname, ts, meta={}):
    with open(fname, "w") as f:
        ...
        ts.to_csv(
            f, date_format="%Y-%m-%dT%H:%M", float_format="%.2f", lineterminator="\n"
        )
...
def parse_srr(df, rate_version=SRR):
    ...
    for d in pdata:
        ...
        if dtx == t0:
            dq = max(dq, 0.0)  # initial out of service marker is status, not a change
        flow = flow_old + dq
        q = d[4]
...
def finalize_and_compare(
    banks_rts, bbid_cfs, qbanks_cdec_daily, stime, etime, interval
):
    ...
    banks_rts[:] = gaussian_filter1d(
        banks_rts, sigma=2, mode="nearest", order=0
    )  # label = "Banks"
    banks_rts.plot(drawstyle="steps-post", color="b", ax=ax0)
    ax0.legend(["Banks processed"])

    qbanks_cdec_daily.plot(
        drawstyle="steps-post", color="r", ax=ax0
    )  # ,label="Banks CDEC daily"
    ts_ave = banks_rts.resample(days(1)).mean()[stime:etime]
    ts_ave.plot(drawstyle="steps-post", color="g", ax=ax0)  # ,label="Banks Daily"

    print(
        "End of data for CDEC: {} and Banks data from script:  {}".format(
            qbanks_cdec_daily.index[-1], banks_rts.index[-1]
        )
    )
    ...
    write_csv(
        "prepared_banks_cms.csv", qbanks_cms, {"station": "Banks", "unit": "f^3s^-1"}
    )
    write_csv(
        "prepared_banks_cfs.csv", qbanks_cfs, {"station": "Banks", "unit": "m^3s^-1"}
    )
    ...
    assert (
        qbbid_cms.index[0] == qbanks_cms.index[0]
    ), "BBID and Banks start dates don't match: %s %s" % (
        qbbid_cms.index[0],
        qbanks_cms.index[0],
    )
    assert (
        qbbid_cms.index[-1] == qbanks_cms.index[-1]
    ), "BBID and Banks end dates don't match: %s %s" % (
        qbbid_cms.index[-1],
        qbanks_cms.index[-1],
    )
    ...
    print(sap_vs_cdec)
    print(
        "Calculating difference between banks daily data on CDEC and processed flows that have been daily averaged"
    )

    banks_diff.plot(linewidth=1.8, color="orange", ax=ax1)
    print(
        "Banks difference time series start %s end %s "
        % (banks_diff.index[0], banks_diff.index[-1])
    )
    ...
    print(
        "Writing exports for %s to %s" % (exports_cms.index[0], exports_cms.index[-1])
    )
    write_csv(
        "prepared_exports_cms.csv",
        exports_cms,
        meta={"station": "swp+bbid", "units": "m^3s-1"},
    )
    write_csv(
        "prepared_exports_cfs.csv",
        exports_cfs,
        meta={"station": "swp+bbid", "units": "m^3s-1"},
    )
    print("Calculating difference between Banks time series as calculated and CDEC")
    print(
        "Filling in qbanks_cdec_daily for nan values with calculated so there will be no difference"
    )
...
def main():
    ...
    banks_df = pd.read_excel(
        banks_xls,
        header=0,
        dtype={"Date": str, "Time": str, "Exclude": str, "AVR": str, "CFS": str},
    )
    # parse_dates=[[5,0]],date_parser=dparse,index_col=("Date_Time","Unit"))

    banks_df["Datetime"] = pd.to_datetime(
        banks_df.Date.str.slice(0, 11)
        + " "
        + banks_df.Time.str.slice(0, 2)
        + ":"
        + banks_df.Time.str.slice(2, 4)
    )

    banks_df = banks_df.set_index(["Datetime", "Unit"])[
        ["Status", "AVR", "PSS", "MWs", "CFS", "Notes", "Exclude"]
    ]
    print(banks_df)
    banks_dss = None
    ...
    try:
        ...
        if banks_dss:
            if not os.path.exists(banks_dss):
                raise ValueError(
                    "DSS file containing banks data does not exist: %s " % banks_dss
                )
            ts = augment_with_dss(uts, stime, etime, interval, banks_dss, banks_path)
        else:
