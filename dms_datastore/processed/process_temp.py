...
freeport_early = read_ts(osp.join(usgs_legacy,"11447650/Temperature,_water.degC.(DATA_PROGRAM)@11447650.EntireRecord.csv"),start=sdate-buffer)
...
verona_early = read_ts(osp.join(usgs_legacy,"11425500/Temperature,_water.degC@11425500.EntireRecord.csv"),start=sdate-buffer)
...
verona_merged = ts_merge([x.squeeze() for x in [knights_t,wilkins,verona_t,verona_early,freeport,freeport_early]])
print(verona_merged)
...
sjr_t = read_ts(osp.join(usgs_dir,"usgs_vns_11303500_temp_*.csv"),start=sdate,end=edate)
print(sjr_t)
...
print("Mokelumne")
nmr_dwr = read_ts(osp.join(ncro_dir,"ncro_mkn_b94133_temp_*.csv"),start=sdate,end=edate)
...
print("American")
watt_t = read_ts(osp.join(usgs_dir,"usgs_awb_11446980_temp_*.csv"),start=sdate,end=edate).squeeze()
...
fairoak_t = read_ts(osp.join(usgs_dir,"usgs_afo_11446500_temp_*.csv"),start = sdate, end=edate).squeeze()
...
temp[columns].to_csv("temp_new.th",date_format="%Y-%m-%dT%H:%M",float_format="%.1f",sep=" ",lineterminator='\n')
