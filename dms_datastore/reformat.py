...
def infer_unit(fname,param,src):
    ...
    thisvar = variable_mappings.loc[(variable_mappings.var_name == param) & (variable_mappings.src_name==src),:]
    print(thisvar)
...
def cdec_unit(fname):
    unit_reformat = {"FEET":"feet","MG/L" : "mg/l","CFS":"ft^3/s","PH":"pH","PSU":"psu","DEG F":"deg_f","DEG C":"deg_c","FT/SEC":"ft/s","uS/cm":"uS/cm","NTU":"NTU","FNU":"FNU"}
...
def reformat():
    ...
    if startupstr is None:
        at_start = True; startupstr = ''
    else:
        at_start = False
    for fpath in allfiles:
        if startupstr in fpath: 
            at_start = True
        if not at_start: continue
