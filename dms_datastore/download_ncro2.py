...
def download_trace(tsrow, row, dest_dir, stime, etime):
    ...
    if station_html is None:
        ...
        meta = ncro_metadata(
            row.station_id, site, site_details, trace_details, paramname
        )
        write_ts_csv(
            df, fpath, metadata=meta, chunk_years=False, format_version="dwr-ncro-json"
        )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        for ndx, row in stations.iterrows():
            ...
            for tsndx, tsrow in subinventory.iterrows():
                ...
                if os.path.exists(os.path.join(dest_dir, proposed_fname)):
                    if not overwrite:
                        logger.info(
                            f"skipping file {proposed_fname} because file exists in dest directory"
                        )
                        continue
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                _ = (
                    future.result()
                )  # This line can be used to handle results or exceptions from the tasks
            except Exception as e:
...
def create_arg_parser():
    ...
    parser.add_argument(
        "--start", default=None, help="Start time, format 2009-03-31 14:00"
    )
