...
def hycom_schism_opendap(start=None, end=None, dest=None):
    ...
    for nday in range(nnday):
        ...
        subset = data.sel(
            time=slice(s.to_datetime64(), e.to_datetime64()),
            lat=slice(37, 39),
            lon=slice(236, 239),
        )
...
def hycom_schism_opendap_alt2(start=None, end=None, dest=None):
    ...
    if (
        start_year == end_year
    ):  ## if the start and end year are the same, we add year to the urls
...
def hycom_schism_opendap_latest(dest=None):
    ...
    for nday in range(nnday):
        ...
        subset = data.sel(
            time=slice(s.to_datetime64(), e.to_datetime64()),
            lat=slice(lat_low, lat_high),
            lon=slice(lon_low, lon_high),
        )
        subset2 = data2.sel(
            time=slice(s.to_datetime64(), e.to_datetime64()),
            lat=slice(lat_low, lat_high),
...
def process_hycom(start=None, end=None, dest="./processed", raw="./raw"):
    ...
    ):  # due to conversion from utc to pst, data for the last day is not available
        end = pd.Timestamp.now()
    ...
    for nday in range(nnday):
        ...
        if (
            len(np.where(raw_nc1.time.diff(dim="time") != np.timedelta64(3, "h"))[0])
            != 0
        ):  # time inverval different from 3hours
            print(
                "%s has inconsistent time interval different from 3 hours " % filename1
            )

        if (
            len(np.where(raw_nc2.time.diff(dim="time") != np.timedelta64(3, "h"))[0])
            != 0
        ):  # time inverval different from 3hours
            print(
                "%s has inconsistent time interval different from 3 hours " % filename2
            )
        ...
        subset1 = resampled1.sel(
            time=slice(s.to_datetime64(), e.to_datetime64() - hours(1))
        )
        subset2 = resampled2.sel(
            time=slice(s.to_datetime64(), e.to_datetime64() - hours(1))
        )
        ...
        merged_set.to_netcdf(
            dest_fn,
            mode="w",
            format="NETCDF4_CLASSIC",
            unlimited_dims=["time"],
            encoding={"salt": {"_FillValue": -9999.0}, "temp": {"_FillValue": -9999.0}},
        )

        assert (
            merged_set.time.shape[0] == 24
        )  # This can occur for the last day downloaded due to conversion from utc to pst.
        s = s + days(1)
...
@click.command()
@click.option(
    "--sdate", default=None, help="Starting date of HYCOM data, format like 2020-02-19"
)
@click.option(
    "--raw_dest", required=True, help="Path to store downloaded raw hycom data"
)
@click.option(
    "--processed_dest", required=True, help="Path to store interpolated hycom data"
)
@click.option(
    "--edate",
    default=None,
...
@click.option(
    "--latest",
    is_flag=True,
    help="If set, download the latest data from the repository, which is +/- 7 days from today.",
)
