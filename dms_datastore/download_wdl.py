...
def wdl_download(
    station_list, years, dest_dir, syear, eyear, overwrite=False, expand_id=True
):
    """Download robot for Water Data Library
    ...
    for ndx, row in station_list.iterrows():
        ...
        for agency_id in station_candidates:
            ...
            if os.path.exists(path):
                ...
                logger.info(
                    f"Attempting to download station: {station} {agency_id} variable {paramname}"
                )
            alias_sub = aliases.loc[
                aliases.FILE_NAME_BASE.str.startswith(items[paramname]), :
            ]
            if alias_sub.shape[0] != 1:
                logger.info(alias_sub)
                raise ValueError(
                    f"Lookup failure on WDL alias table (nonunique) for station {agency_id} and param {param}"
                )
            file_created = False
            ndata = 0
            data_years = []
            ...
            for year in years:
                ...
                station_query = "{}/{}/{}/{}".format(
                    base_url, agency_id.upper(), year, filename
                )
            ...
            if not file_created:
                fails.append((station, paramname))
    logger.info("Fails:")
    for fl in fails:
        logger.info(fl)
...
def download_wdl(
    ...
    wdl_download(df, destdir, destdir, syear, eyear, overwrite, expand_id)
