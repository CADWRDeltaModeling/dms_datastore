...
freeport = read_ts(
    osp.join(usgs_dir, "usgs_fpt_11447650_temp_2007_202*.rdb"), selector="15731_00010"
).squeeze()
...
knights_t2 = (
    pd.read_csv(
        osp.join(wdl_dir, "SacRabCBD_2021-11-17_110212_-0800.csv"),
        header=1,
...
sjr_t = read_ts(
    osp.join(usgs_dir, "usgs_vns_11303500_temp_2007_2022.rdb"),
    selector="15168_00010",
    start=sdate,
    end=edate,
)
...
sjr_des_t = read_ts(
    osp.join(des_dir, "des_sjr_90_temp_2007_9999.csv"), start=sdate, end=edate
)
sjr_des_t.columns = ["sjr"]
sjr_des_t = sjr_des_t.interpolate(limit=8)

sjr_cdec_t = read_ts(
    osp.join(cdec_dir, "cdec_ver_*temp_2007_*.csv"), start=sdate, end=edate
)
...
print("Mokelumne")
nmr_dwr = read_ts(
    osp.join(wdl_dir, "ncro_mkn_b94133_temp_2007_2022.csv"), start=sdate, end=edate
)
...
print("American")
watt_t = read_ts(
    osp.join(usgs_dir, "usgs_awb_11446980_temp_2007_2022.rdb"), start=sdate, end=edate
).squeeze()
watt_t = watt_t.interpolate(limit=20)
watt_t.columns = ["american"]
fairoak_t = read_ts(
    osp.join(usgs_dir, "t_fair_oaks_usgs.rdb"), start=sdate, end=edate
).squeeze()
