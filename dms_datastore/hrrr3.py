...
class AWSGrib2Inventory:
    ...
    def __init__(
        ...
        pages = paginator.paginate(
            Bucket=self.bucket,
            Prefix=f'hrrr.{self.forecast_cycle.strftime("%Y%m%d")}' f"/{self.product}/",
        )
        data = []
        for page in pages:
    ...
    def output_interval(self) -> timedelta:
        return {"conus": timedelta(hours=1)}[self.product]
    ...
    def files(self):
        grbfiles = glob.glob(
            f'{self.tmpdir}/hrrr.{self.forecast_cycle.strftime("%Y%m%d")}/conus/hrrr.t{self.cycle:02d}z.wrfsfcf*.grib2'
        )
        grbfiles.sort()
        return grbfiles
...
class HRRR:
    ...
    def __init__(self, start_date=None, rnday=None, pscr=None, record=1, bbox=None):
        ...
        datevector = np.arange(
            start_date, end_date, np.timedelta64(1, "D"), dtype="datetime64"
        )
    ...
    def gen_sflux(self, date, record, pscr):
        ...
        Vars = {
            ...
            "group3": {
                "prate": ["surface", prate, "prate"],
                "dlwrf": ["surface", dlwrf, "sdlwrf"],
                "dswrf": ["surface", dswrf, "sdswrf"],
            },
        }
                ...
                if key == "group1":
                    for key2, value2 in value.items():
                        try:
                            ...
                            value2[1].append(
                                ds[key2][
                                    idx_ymin : idx_ymax + 1, idx_xmin : idx_xmax + 1
                                ].astype("float32")
                            )
                            ds.close()
                        except Exception:
                            logger.info(
                                "paramId " + value2[0] + " not found in %s" % file
                            )
                            print("paramId " + value2[0] + " not found in %s" % file)
                            value2[1].append(
                                np.tile(
                                    np.nan,
                                    (idx_ymax - idx_ymin + 1, idx_xmax - idx_xmin + 1),
                                )
                            )
        ...
        fout = xr.Dataset(
            ...
            coords={
                "time": np.round(np.arange(1, len(grbfiles) + 1) / 24, 4).astype(
                    "float32"
                ),
        ...
        fout.prate.attrs = {"units": "kg m-2 s-1", "long_name": "Precipitation rate"}
        fout.dlwrf.attrs = {
            "units": "W m-2",
        ...
        fout.to_netcdf(
            path=f'{pscr}/{date.strftime("%Y")}/hrrr_{date.strftime("%Y%m%d")}{cycle:02d}.nc',
            mode="w",
            format="NETCDF4",
            engine="netcdf4",
            unlimited_dims="time",
