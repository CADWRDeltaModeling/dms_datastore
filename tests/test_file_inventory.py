...
def test_repo_file_inventory_groups_by_file_pattern(monkeypatch):
    ...
    monkeypatch.setattr(
        inventory,
        "coerce_repo_config",
        lambda repo=None, repo_cfg=None: repo_cfg or repo,
    )
    monkeypatch.setattr(
        inventory,
        "repo_registry",
        lambda repo=None, repo_cfg=None: registry,
    )
...
def test_repo_data_inventory_groups_by_series_id(monkeypatch):
    ...
    assert row["max_year"] == "2024"
    assert row["name"] == "Antioch"
    assert row["file_pattern"] in {
        "*_anh@north_11303500_flow_*.csv",
        "*_anh@north_B12345_flow_*.csv",
    }